        ...
        button : function() {
            if (_s.measure == 'Manski_sliders'){
                ...
                if(_s.measure == 'Manski_1' |
                    ...
                    manski_trial_number = parseInt(_s.measure[7]);
                    response_data_tag = 'M' + manski_trial_number + '_response_data';
                    if (_s[response_data_tag] != null){
                        //if rating is 100 or if only 1 number to rate
                        if (_s[response_data_tag] == 100 || numM.length == manski_trial_number) {
                            //jump to next Manski slider task, see function Stream-V2.js
                            for (var i=manski_trial_number; i<=5; i++) {
                                // starting at this slide number, keep moving on until you've gotten past slide 5
                                stim = this.present.shift();
                            }
                            _s.log_responses();
                            _stream.apply(this);
                        } else {
                            //if rating is not 100, just log this response and move on
                            _s.log_responses();
                            _stream.apply(this);
                        }
                    } else {
                        //if rating is uncooperative, show an error
                        $("#itemError" + manski_trial_number).show();
                    }
