...
\author{{\large \bf Michael Franke} \\
 ...
 {\large \bf Erin Bennett, Judith Degen, Michael Henry Tessler, Noah D. Goodman}\\
     \{ebennett, jdegen, mtessler, ngoodman\}@stanford.edu \\
...
$w$ to determine how much perturbation we should expect.
 To realize this, the model assumes
...
Finally, consider the link function for \textbf{bin comparisons}. We are interested in the likelihood
...
what participants were doing. Indeed, a model that implements this idea blatantly failed to capture the
relevant regularities in the data. Another plausible link function is to assume that
...
the prototype.\footnote{Notice that
  ...
  \mht{I am unable to notice this. Double check? Intuition tells me $p^\text{low}_{ijl} = 1 - p^\text{high}_{ijl}$.}
  }
\begin{align*}
...
ample variation between participants. Nonetheless, the way the model suggests we should think about
...
posterior predictions are spot-on. For the give-a-number task, some of the data is surprising despite 
the model being trained on this very data. This could have various reasons: (i) the give-a-number data does
not have a huge influence on the posterior likelihood, (ii) number choices may be influenced by
saliency and/or roundness of numbers after all (and thus, not accurately reflect the true beliefs $Q_j$). Finally, there is one condition in the paired
comparison task that the model definitely got wrong. This is the choice of what is more likely:
One or none of 14 marbles thrown into a pool would sink. The model would predict that
...
\mht{Is it worth mentioning about the possilbiity of including subject-specific link function parameters e.g., $\kappa$ and $\alpha$?}
\section{Conclusion}
