...
\begin{tikzpicture}[node distance = 2.5cm, 
    ...
    \begin{pgfonlayer}{background}
       \node [thick, dotted,
              draw=black!90,fit={($(sijk.south)+(0,-17pt)$) 
                                 ($(sijk.west)+(-35pt,-5pt)$) 
                                 ($(Qj.north)+(0,+5pt)$) 
                                 ($(sijk.east)+(+0pt,0)$)}] {};
    \end{pgfonlayer}

    \node[below of = sijk, node distance = 1cm+3pt] (KlabelAnchor) {};

    \node[left of = KlabelAnchor, node distance = 0.25cm+10pt] (Klabel)
    {bin $k \in \set{0, \dots, 15}$};
    ...
    \begin{scope}[xshift = 9.75cm, node distance = 1cm]
      ...
      \node[below of = nij] (cijl) {$c_{ijl} \sim
        \text{Bern}(\expo(b \ pc_{ijl}))$, where};

      \node[below of = cijl, node distance = 1.35cm] (pcijl) {$pc_{ijk} = \begin{cases} 2 &
          \text{if $mode(P_{ij})$ is closer to higher} \\
